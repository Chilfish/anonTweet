...
- **输入**：`app/components/tweet/` 目录组件清单与 `app/stories/*.stories.tsx` 清单
- **验证方法**：静态盘点（`test/acceptance/ac-ui.spec.ts`）——评审清单 14 个 tweet 组件逐一核对其
  story 文件存在且非空；tweet 目录组件文件数 ≥ 14；story 文件总数 ≥ 18（4 旧 + 14 新）。
- **Pass 条件**：
...
- **开放问题**：`MyTweet`（`Tweet.tsx`，评论区页组合组件）不在评审 14 清单内，由
  `CommentBranch` 覆盖其渲染路径；是否单列 story 由覆盖盘点（阶段二第 4 周）决定。
