...
When using object values, the `SelectItem` component receives the entire object as its `value` prop. The `SelectValue` component will automatically display the correct label if the objects are properly mapped.
