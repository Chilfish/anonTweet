...
- [ ] [ux] Twitter 推文搜索（方案：`docs/features/tweet/search.md`；文件：`app/lib/rettiwt-api/collections/Groups.ts`、`app/lib/react-tweet/utils/get-tweet.ts`、`app/routes/api/tweet/search.ts`、`app/routes/search.tsx`、`app/components/layout/Layout.tsx`；工作量：1-2 人日 / 风险：中，guest 搜索可能受限）— 🔄 进行中（2026-08-31，分支 `feat/tweet-search`）：rettiwt-api 底层搜索已具备，补 `TWEET_SEARCH` guest 授权组 + App 数据层/BFF/页面接入；AC-TWEET-009/010 先行落地（见 AC-tweet.md）
