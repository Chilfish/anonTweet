...
- [ ] [refactor] 门禁命令统一与集成测试守卫（关联：review P3-1；文件：`CLAUDE.md`、`README.md`、`test/integration/*.spec.ts`；工作量：0.5 人日 / DRI：维护者 / 风险：低）— CLAUDE.md 强制命令 `bun test` 改指 `bun run test`；`testEnv` 无服务器时 `describe.skipIf` 对齐凭据守卫
- [ ] [refactor] SmartPool 限流/冷却/失败隔离重构 + 单测（关联：review P1-1；文件：`app/lib/SmartPool.ts`、`test/unit/smart-pool.spec.ts`；工作量：2-3 人日 / DRI：核心后端 / 风险：中，改核心链路需 mock 回归）— per-key `cooldownUntil` / in-flight 计数 / 单 Key 指数退避
- [ ] [refactor] Translation View Resolver 收敛（原 backlog L24 采纳；关联：review P2-3；文件：`app/hooks/use-translation-editor-logic.ts`、`use-alt-translation-logic.ts`、`app/lib/translation/resolveEntities.ts`；工作量：1-2 人日 / 风险：中，hot files）— 两 hook 复用 `deriveManualTranslation` 纯函数，spec 断言三处结果一致
- [ ] [refactor] `catch any` / 空 catch 清零（关联：review P2-2；文件：`app/routes/api/tweet/get.ts` L54/L142、`ai-translation.ts` L118/L142/L190、`bili-post.tsx` L111；工作量：1 人日 / 风险：低）— `catch (error: unknown)` + `instanceof Error`；空 catch 补结构化 console
- [ ] [ux] Bili 隐藏功能卫生化（关联：review P2-1；文件：`app/routes/api/bili-post.tsx`、`docs/INDEX.md`、`docs/planning/project-architecture.md`；工作量：1 人日 / 风险：低）— `ENABLE_BILI` 开关（默认关）+ 文档标注"隐藏自用入口，非产品功能，不宣传" + IP 暴露缺陷（project-architecture.md L94）记已知限制
