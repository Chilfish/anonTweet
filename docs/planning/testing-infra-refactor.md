...
### Phase A — Vitest 三层骨架 + 共享工具（~0.5 天）
...
5. `scripts/verify-runner.ts`（或薄 `verify/index.ts` 改造）：AC 过滤、✓/✗ reporter、exit code

**验收**：`bun run verify` 等价输出旧 CLI（57 AC 对应测试全部绿）；`bun run test` 不受影响
**commit 拆分**：`docs:` 本计划 → `chore:` vitest projects 配置 → `test:` 共享 helpers + SDK/fixtures 迁移
