...
export async function fetchUserTimeline(userId: string): Promise<RawTweet[]> {
  return twitterPool.run(async fetcher => fetcher
...
export async function fetchUserTimelineWithReplies(userId: string): Promise<RawTweet[]> {
  return twitterPool.run(async fetcher => fetcher
...
export async function getEnrichedUserTweet(userId: string): Promise<EnrichedTweet[]> {
  ...
  // await writeFile('cache/user-timeline-tweets.json', JSON.stringify(tweets, null, 2), 'utf8')
  return tweets
    .map((tweet) => {
