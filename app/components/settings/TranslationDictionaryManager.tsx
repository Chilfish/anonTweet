...
type ImportOutcome
  = | { type: 'success', parsed: Omit<TranslationDicEntry, 'id' | 'createdAt'>[] }
    | { type: 'empty' }
    | { type: 'error', error: unknown }

async function parseImportedExcel(result: ArrayBuffer): Promise<ImportOutcome> {
  try {
    ...
    return { type: 'error', error: new Error('Invalid format') }
  }
  catch (error) {
...
function DictionaryAddSection() {
  ...
  const handleAdd = () => {
    if (!newOriginal.trim() || !newTranslated.trim())
      return
    addEntry({ original: newOriginal.trim(), translated: newTranslated.trim() })
    setNewOriginal('')
    setNewTranslated('')
    toastManager.add({ title: '已添加', type: 'success' })
  }

  const handleExport = async () => {
    try {
      await downloadExcel(entries)
      toastManager.add({ title: '导出成功', description: 'Excel 文件已开始下载', type: 'success' })
    }
    catch (e) {
      console.error('Export failed', e)
      toastManager.add({ title: '导出失败', description: `导出过程中发生错误: ${e}`, type: 'error' })
    }
  }
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      if (!result) {
        toastManager.add({ title: '导入失败', description: '文件读取失败。', type: 'error' })
        if (fileInputRef.current)
          fileInputRef.current.value = ''
        return
      }

      const outcome = await parseImportedExcel(result)

      if (outcome.type === 'success') {
        const stats = importEntries(outcome.parsed)
        toastManager.add({
          title: '导入成功',
        ...
        console.error('Import failed', outcome.error)
        toastManager.add({ title: '导入失败', description: '解析 Excel 失败，请检查文件格式。', type: 'error' })
      }
...
function DictionaryEditDialog({
  ...
  entry: TranslationDicEntry | null
  isOpen: boolean
  onClose: () => void
}) {
  ...
  const handleSave = () => {
    if (entry && editOriginal.trim() && editTranslated.trim()) {
      updateEntry(entry.id, { original: editOriginal.trim(), translated: editTranslated.trim() })
      onClose()
    }
  }

  return (
    <Dialog open={isOpen} onOpenChange={onClose} dismissible={false}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>编辑词汇</DialogTitle>
          <DialogDescription>修改词汇的对应关系。</DialogDescription>
        </DialogHeader>
...
export function TranslationDictionaryManager() {
  ...
  return (
    <div className="space-y-6 p-1">
      <DictionaryAddSection />
      <DictionaryListSection onEdit={openEditDialog} />
